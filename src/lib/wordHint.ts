...
function guessPos(word: string): string {
  ...
  if (key.endsWith("ing") && key.length > 4) return "动词";
  if (key.endsWith("ed") && key.length > 3) return "动词";
  if (/(tion|sion|ness|ment|ity|ship|hood)$/.test(key)) return "名词";
  if (/(ous|ful|less|ish|ive|able|al|ic|ical)$/.test(key) && key.length > 4) return "形容词";
  if (key.endsWith("s") && key.length > 3 && !key.endsWith("ss") && !key.endsWith("us")) {
    const singular = key.endsWith("ies")
      ? `${key.slice(0, -3)}y`
      : key.endsWith("es")
        ? key.slice(0, -2)
        : key.slice(0, -1);
    if (LOCAL_POS[singular]) return LOCAL_POS[singular];
    return "名词";
  }
  return "";
}
